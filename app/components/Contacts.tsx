  ...
  return (
    ...
    {contacts.map((contact, index) => (
        <li key={index} className={`contact ${contact.selected ? 'selected-contact' : ''} flex flex-row justify-start lg:justify-center items-center border-b border-[#CCCCF5] w-full py-2 px-5 lg:p-2 cursor-pointer`} onClick={() => handleClick(index)}>
            <div className="contact-profile mr-1 lg:mr-3 lg:block rounded-full">
                {contact.contact.image && <Image className='rounded-full' src={contact.contact.image} alt={contact.contact.name} width={50} height={50} />}
            </div>
            <div className="contact-details w-full md:w-10/12 lg:w-9/12 overflow-hidden">
                <div className="name-time flex flex-row justify-between items-center">
                    <p className='contact-name font-bold text-xs lg:text-base'>{contact.contact.name}</p>
                    <p className='contact-time text-xs lg:text-sm text-right'>{contact.lastMessageTime || ''}</p>
                </div>
                <p className='contact-last-message text-nowrap text-sm overflow-hidden text-ellipsis my-1'>{contact.lastMessage || `start ur conversation with ${contact.contact.name} now`}</p>
            </div>
        </li>
    ))}
    </ul>
